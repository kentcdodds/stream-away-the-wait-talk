...
// node_modules/@remix-run/react/esm/transition.js
var DeferredResponse = class {
  constructor(response) {
    this.response = response;
  }
};
var DEFERRED_PROMISE_PREFIX = "__deferred_promise:";
async function parseDataFromDeferredReadableStream(body) {
  if (!body) {
    return {
      initialData: void 0,
      deferred: {}
  ...
  let readTheRestOfTheResponse = async () => {
    for (let section = await readStreamSection(); section; section = await readStreamSection()) {
  ...
  if (!initialSection)
    throw new Error("No initial deferred data found.");
  ...
  if (typeof initialData === "object" && initialData !== null) {
    for (let [eventKey, value] of Object.entries(initialData)) {
      ...
      deferred[eventKey] = new Promise((resolve) => {
        deferredResolvers[eventKey] = (value2) => {
          resolve(value2);
  ...
  readTheRestOfTheResponse().catch((error) => {
    for (let [key, resolver] of Object.entries(deferredResolvers)) {
      resolver(error);
      delete deferredResolvers[key];
    }
...
var TransitionRedirect = class {
  constructor(location, setCookie = false) {
...
function createTransitionManager(init) {
  ...
  let pendingNavigationDeferredControllers = /* @__PURE__ */ new Map();
  let cancelledDeferredRouteIds = /* @__PURE__ */ new Set();
  ...
  function update(updates) {
    ...
    if (updates.transition) {
      if (updates.transition === IDLE_TRANSITION) {
        ...
        overrides.deferredLoaderData = [...Object.entries(state.deferredLoaderData), ...Object.entries(updates.deferredLoaderData || {})].filter(([routeId]) => !cancelledDeferredRouteIds.has(routeId) || pendingNavigationDeferredControllers.has(routeId)).reduce((acc, [routeId, data]) => Object.assign(acc, {
          [routeId]: data
        }), {});
        cancelledDeferredRouteIds.forEach((id) => cancelledDeferredRouteIds.delete(id));
      }
    }
  ...
  function dispose() {
    ...
    abortPendingDeferredControllers(pendingNavigationDeferredControllers, cancelledDeferredRouteIds);
  ...
  async function handleActionFetchSubmission(key, submission, match) {
    ...
    let results = await callLoaders(state, state.transition.location || state.location, matchesToLoad, controller.signal, pendingNavigationDeferredControllers, cancelledDeferredRouteIds, maybeActionErrorResult, maybeActionCatchResult, submission, match.route.id, loadFetcher);
    if (controller.signal.aborted) {
      return;
    }
    fetchReloadIds.delete(key);
    ...
    let [deferredLoaderData, monitorDeferred] = makeDeferredLoaderData(getState, update, results, pendingNavigationDeferredControllers);
    if (yeetedNavigation) {
      let {
        transition
      } = state;
  ...
  async function loadPageData(location, matches2, submission, submissionRouteId, actionResult, catchVal, catchBoundaryId) {
    ...
    let results = await callLoaders(state, location, matches2, controller.signal, pendingNavigationDeferredControllers, cancelledDeferredRouteIds, maybeActionErrorResult, maybeActionCatchResult, submission, submissionRouteId, void 0, catchBoundaryId);
    if (controller.signal.aborted) {
      return;
    }
    let redirect = findRedirect(results);
...
async function callLoaders(state, location, matches, signal, pendingNavigationDeferredControllers, cancelledDeferredRouteIds, actionErrorResult, actionCatchResult, submission, submissionRouteId, fetcher, catchBoundaryId) {
  let url = createUrl(createHref(location));
  let matchesToLoad = filterMatchesToLoad(state, location, matches, cancelledDeferredRouteIds, actionErrorResult, actionCatchResult, submission, submissionRouteId, fetcher, catchBoundaryId);
  abortPendingDeferredControllers(pendingNavigationDeferredControllers, cancelledDeferredRouteIds, matches, matchesToLoad);
...
async function callLoader(match, url, signal) {
  ...
  try {
    ...
    let response = await match.route.loader({
      params,
      url,
      signal
    });
    ...
    if (response instanceof DeferredResponse) {
      let parsed = await parseDataFromDeferredReadableStream(response.response.body);
...
async function callAction(submission, match, signal) {
  invariant2(match.route.action, `Expected action for ${match.route.id}`);
  try {
...
function filterMatchesToLoad(state, location, matches, cancelledDeferredRouteIds, actionErrorResult, actionCatchResult, submission, submissionRouteId, fetcher, catchBoundaryId) {
  var _location$state;
...
function makeDeferredLoaderData(getState, update, results, pendingNavigationDeferredControllers) {
  let state = getState();
  let deferredLoaderData = {
  ...
  for (let {
    ...
    if (!deferred)
      continue;
    deferredLoaderData[match.route.id] = Object.assign({}, deferredLoaderData[match.route.id], deferred);
    let deferredController = new AbortController();
    pendingNavigationDeferredControllers.set(match.route.id, deferredController);
  }
  return [deferredLoaderData, () => {
    for (let {
      match,
      deferred
    } of results) {
      ...
      for (let [key, promise] of Object.entries(deferred)) {
        var _pendingNavigationDef;
        let signal = (_pendingNavigationDef = pendingNavigationDeferredControllers.get(match.route.id)) === null || _pendingNavigationDef === void 0 ? void 0 : _pendingNavigationDef.signal;
        ...
        promise.then((value) => {
          ...
          let {
            [match.route.id]: {
              [key]: _,
              ...routeDeferredLoaderData
            } = {},
            ...deferredLoaderData2
          } = state2.deferredLoaderData;
          let newDeferredLoaderData = {
            ...deferredLoaderData2,
            [match.route.id]: routeDeferredLoaderData
          };
          let {
            [match.route.id]: routeLoaderData,
            ...loaderData
          } = state2.loaderData;
          let newLoaderData = {
            ...loaderData,
...
function abortPendingDeferredControllers(pendingNavigationDeferredControllers, cancelledDeferredRouteIds, matches = [], matchesToLoad = []) {
...
async function resolveDeferredData({
  ...
  if (typeof resolvedValue === "object" && resolvedValue && typeof deferred === "object") {
    let deferredPromises = Object.entries(deferred).map(async ([key, promise]) => {
  ...
  let loader = async ({
    ...
    if (route.hasLoader) {
      ...
      if (result instanceof Error)
        throw result;
      let response = result instanceof DeferredResponse ? result.response : result;
      let redirect = await checkRedirect(response);
      if (redirect)
        return redirect;
      if (isCatchResponse(response)) {
        throw new CatchValue(response.status, response.statusText, await extractData(response));
      }
...
function createAction(route, routeModules) {
  let action = async ({
    ...
    let redirect = await checkRedirect(result instanceof DeferredResponse ? result.response : result);
    if (redirect)
      return redirect;
    await loadRouteModuleWithBlockingLinks(route, routeModules);
...
var DEFERRED_PROMISE_PREFIX2 = "__deferred_promise:";
var deferredContext = /* @__PURE__ */ React3.createContext(void 0);
var js = String.raw;
function Deferred({
  children,
  value,
  ...
  let suspenseDataKey = void 0;
  if (typeof value === "string" && value.startsWith(DEFERRED_PROMISE_PREFIX2)) {
    suspenseDataKey = value.slice(DEFERRED_PROMISE_PREFIX2.length);
  }
  if (suspenseDataKey) {
    value = routeDataDeferred[id][suspenseDataKey];
  }
  return /* @__PURE__ */ React3.createElement(deferredContext.Provider, {
    value: {
      value,
      key: suspenseDataKey
    }
    ...
    fallback
  }, /* @__PURE__ */ React3.createElement(DeferredErrorBoundary, {
    errorElement,
    nonce
  }, typeof children === "function" ? /* @__PURE__ */ React3.createElement(ResolveDeferred, {
    children
  }) : children)), suspenseDataKey && /* @__PURE__ */ React3.createElement("script", {
    nonce,
    dangerouslySetInnerHTML: {
      __html: js`(() => {
  ...
  window.__remixContext.deferredRouteDataResolvers[${JSON.stringify(id)}] = Object.assign(
    {},
...
function useDeferredData() {
  let ctx = React3.useContext(deferredContext);
  return ctx === null || ctx === void 0 ? void 0 : ctx.value;
}
...
function DeferredErrorBoundary({
  ...
  let ctx = React3.useContext(deferredContext);
  invariant2(ctx, "Deferred must be used inside a DeferredProvider");
  let {
    value
  } = ctx;
  let child = children;
  ...
  if (value instanceof Error) {
    ...
    child = typeof errorElement === "object" && errorElement && "type" in errorElement && typeof errorElement.type === "function" ? /* @__PURE__ */ React3.cloneElement(errorElement, {
      error: value
    }) : errorElement;
  }
  return /* @__PURE__ */ React3.createElement(React3.Fragment, null, child, /* @__PURE__ */ React3.createElement(DeferredHydrationScript, {
    nonce
  }));
}
function DeferredHydrationScript({
  ...
  if (!key)
    return null;
  let preHydration = "", hydrationValue;
  if (value instanceof Error) {
    preHydration = `let error = new Error(${JSON.stringify(value.message)});error.stack = ${JSON.stringify(value.stack)};`;
...
window.__remixContext.deferredRouteDataResolvers[${JSON.stringify(id)}][${JSON.stringify(key)}](
  window.__remixContext.deferredRouteData[${JSON.stringify(id)}][${JSON.stringify(key)}]
);
})();`
    }
  });
...
var NavLink2 = /* @__PURE__ */ React3.forwardRef(({
  ...
  return /* @__PURE__ */ React3.createElement(React3.Fragment, null, /* @__PURE__ */ React3.createElement(NavLink, _extends2({
    ref: forwardedRef,
    to
  }, props, typeof prefetchHandlers === "object" ? {
    ...prefetchHandlers
  } : {})), shouldPrefetch ? /* @__PURE__ */ React3.createElement(PrefetchPageLinks, {
    page: href
  }) : null);
});
...
var Link2 = /* @__PURE__ */ React3.forwardRef(({
  ...
  return /* @__PURE__ */ React3.createElement(React3.Fragment, null, /* @__PURE__ */ React3.createElement(Link, _extends2({
    ref: forwardedRef,
    to
  }, props, typeof prefetchHandlers === "object" ? prefetchHandlers : {})), shouldPrefetch ? /* @__PURE__ */ React3.createElement(PrefetchPageLinks, {
    page: href
  }) : null);
});
...
function Scripts(props) {
  ...
  let initialScripts = React3.useMemo(() => {
    let contextScript = serverHandoffString ? `window.__remixContext = Object.assign({}, ${serverHandoffString}, window.__remixContext);` : "";
...
/**
 * @remix-run/react v0.0.0-experimental-708e229e
 *
 ...
 * @license MIT
 */
//# sourceMappingURL=/build/_shared/chunk-HJQWB46P.js.map
